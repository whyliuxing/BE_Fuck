...
std::ofstream LogFile = std::ofstream("D:\\MyProjects\\VSProjects\\BE_Fuck\\Debug\\log.txt");
...
BOOL KillBattEye() {
	...
	TerminateProcess(OpenProcess(PROCESS_ALL_ACCESS, 0, GetProcessIdByName(GAME_NAME)), 0);
...
DWORD WINAPI PipeHandleThread(LPVOID lpParam)
{
	...
	while (WaitForSingleObject(g_ServiceStopEvent, 0) != WAIT_OBJECT_0)
	{
		...
		if (dwReaded)
		{
			...
			LogFile << "│ [ID: " << (int)BattlEyeData->ID << "][Recv] [" << dwWritten << " bytes]\n" << OutputBuffer.str() << std::endl;
			LogFile.flush();
			...
			if (BattlEyeData->ID == 0)
			{
				...
				// trans to old
				if (false && packet_6[0] != 6 && (g_PipeHandle_Old != 0 || connectToOldPipe(g_PipeHandle_Old))) {
			...
			else if (BattlEyeData->ID == 6)
			{
				...
				if (packet_6[0] != 6 && (g_PipeHandle_Old != 0 || connectToOldPipe(g_PipeHandle_Old))) {
...
#pragma endregion
				...
				if (packet_6[0] == 6) {
					...
					//unturned(old)
					//BYTE static_pPacket[] = { 0x06,0x7c,0x40,0xe0,0x46,0x83,0xc3,0x23,0x19,0x56,0xea,0x4e,0x75,0xb5,0x08,0x24,0x0c,0xe8,0x90,0xf5,0x8e,0x2f,0x80,0x4d,0x79,0xb4,0xf5,0xf7,0x8a,0x4a,0xf7,0x24,0x70,0xa8,0x70,0x4c,0x71,0xd0,0x7f,0x4d,0x05,0xc0,0x66,0x5c,0x60,0xc0,0x46,0x7c,0x3c,0xa0,0xa6,0xc5,0xbf,0x1f,0xb9,0x7c,0x3c,0xa0,0xa6,0xc5,0xbf,0x1f,0xb9,0x7c,0x40,0xe0,0x46,0x7c,0xc3,0x23,0x65,0xe6,0x59,0xb9 };
...
#pragma region old_Packet_2
				if (packet_2[0] != 2 && g_PipeHandle_Old != 0) {
...
#pragma endregion
			...
			else if (BattlEyeData->Data[0] == 0x05 && (BattlEyeData->Data[1] == 0x06 || BattlEyeData->Data[1] == 0x04 || BattlEyeData->Data[1] == 0x07) 
		...
		if (enterLoop)
		{
			...
			while (WriteFile(g_PipeHandle, &Request_5, 1, &dwWritten, 0)) {
				MyOutputDebugString("-------BattlEye request %d-------", 5);
				...
				if (sleepCount >= 30000) {
					...
					WriteFile(g_PipeHandle, &packet_2, 5, &dwWritten, 0);
					hex_dump(packet_2, dwWritten, OutputBuffer);
					...
					*reinterpret_cast<DWORD*>(packet_2b + 1) = dwGameID;
					WriteFile(g_PipeHandle, &packet_2b, 5, &dwWritten, 0);
